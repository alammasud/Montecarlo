...
int main(){
	...
	int tt=0;
	//while(tt<2){
	fscanf(fp," %i %s",&ntot,item);
	fscanf(fp," %i %s %s",&step,item,item);
	//fscanf(fp," %s %s %s %s",item21,item22,item23,item24);
	//fscanf(fp," %i",&ntot);
	//fscanf(fp," %s %s %s %s %s %s",item31,item32,item33,item34,item35,item36);
	fscanf(fp,"%lf %lf %s %s",&xlow,&xhi,item,item);
	fscanf(fp,"%lf %lf %s %s",&ylow,&yhi,item,item);
	fscanf(fp,"%lf %lf %s %s",&zlow,&zhi,item,item);
        fscanf(fp,"%s",item);
	...
	for(i=0;i<ntot;i++){
		fscanf(fp,"%i %i %lf %lf %lf",&index[i],&type[i],&x[i],&y[i],&z[i]);
	}
	//tt++;
//	}
	fclose(fp);
	...
	for(i= 0;i<ntot;i++){
		fprintf(fp2,"%i\t%i\t%20.20lf\t%20.20lf\t%20.20lf\n",index[i],type[i],x[i],y[i],z[i]);
	}
	fclose(fp2);
	FILE* haire;
